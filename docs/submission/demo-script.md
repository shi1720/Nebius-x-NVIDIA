...
| 1:57–2:15 | Show alternate-lot branch and reviewer controls; do not release stock.                          | “Another batch of three hundred sixty oat bars references lot PB-0901-B. That distinction matters, but being outside this supplied lineage is not a general safety clearance. The workspace preserves the evidence and the review decision, so someone else can understand what happened.”                                                   |
...
| 2:38–2:55 | Return to main workspace with counts, graph, and open-issue marker visible. End on title.       | “RecallRoom is for smaller teams working with the records they already have. Our next step is to validate real drill workflows with quality professionals. The goal is simple: a recall response that people can inspect, explain, and act on.”                                                                                              |
...
## Optional stronger ambiguity shot

If the final build supports evidence-backed resolution, create a separate, clearly labeled synthetic correction document and record the reviewer applying it. Only change the narration and final totals after recomputing the case and confirming the expected results. Do not silently decide which lot the cookies used for visual convenience.
